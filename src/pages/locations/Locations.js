...
import { PaginationWithSize } from '../../components/shared/PaginationWithSize'; // 👈 shared component
...
// BRANCHES TAB
// ═══════════════════════════════════════════════════════════════════════════════
const branchCols = { name: 'Branch Name', address: 'Address', status: 'Status' };
...
function BranchesTab() {
  ...
  const visibleColumns = useMemo(() => {
    ...
    return Object.keys(branchCols).filter(col =>
      controls.paginated.some(row => { const val = getBranchVal(row, col); return val && val !== '-'; })
    );
    ...
    <Box>
      ...
      <Grow in timeout={900}>
        <Card>
              ...
              <TableHead>
                <TableRow>
                  {visibleColumns.map((col, index) => (
                    <SortFilterHeader key={col} label={branchCols[col]} column={col}
                      sticky={index === 0}
                  ...
                  <TableRow>
                    <TableCell colSpan={visibleColumns.length + 2} align="center" sx={{ py: 6 }}>
                      ...
                      {Object.values(controls.filters).flat().length === 0 && !controls.globalSearch && (
                        <Button variant="contained" size="small" startIcon={<AddIcon />} onClick={() => navigate('/locations/branches/create')}>Create Branch</Button>
                      )}
                    </TableCell>
                  </TableRow>
                ) : (
          ...
          {/* 👇 Shared PaginationWithSize */}
          <PaginationWithSize
            page={controls.page} totalPages={controls.totalPages}
            totalCount={controls.filtered.length} pageSize={controls.pageSize}
            onPageChange={controls.setPage} onPageSizeChange={controls.setPageSize} />
        </Card>
      </Grow>
...
// PROJECT SITES TAB
// ═══════════════════════════════════════════════════════════════════════════════
const siteCols = { name: 'Project Name', code: 'Code', status: 'Status' };
...
function ProjectSitesTab() {
  ...
  const visibleColumns = useMemo(() => {
    ...
    return Object.keys(siteCols).filter(col =>
      controls.paginated.some(row => { const val = getSiteVal(row, col); return val && val !== '-'; })
    );
          ...
          <TableContainer sx={{ overflowX: 'auto' }}>
            <Table stickyHeader sx={{ tableLayout: 'auto' }}>
              <TableHead>
                <TableRow>
                  {visibleColumns.map((col, index) => (
                    <SortFilterHeader key={col} label={siteCols[col]} column={col}
                      sticky={index === 0}
                      sortColumn={controls.sortColumn} sortDirection={controls.sortDirection}
                      filterCount={controls.filters[col]?.length || 0}
                      onSort={controls.handleSortClick} onFilter={controls.handleFilterClick} />
                ...
                {controls.paginated.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={visibleColumns.length + 2} align="center" sx={{ py: 6 }}>
                      ...
                      {Object.values(controls.filters).flat().length === 0 && !controls.globalSearch && (
                        <Button variant="contained" size="small" startIcon={<AddIcon />} onClick={() => navigate('/locations/sites/create')}>Create Project Site</Button>
                      )}
                    </TableCell>
                  </TableRow>
                ) : (
...
export default function Locations() {
  ...
  const handleTabChange = (_, newVal) => { setSearchParams({ tab: newVal === 1 ? 'sites' : 'branches' }, { replace: true }); };
        ...
        <PageHeader
          ...
          actions={
            <Button variant="contained" startIcon={<AddIcon sx={{ display: { xs: 'none', sm: 'inline-flex' } }} />} size="large"
              ...
              <Box component="span" sx={{ display: { xs: 'none', sm: 'inline' } }}>{isSitesTab ? 'Create Project Site' : 'Create Branch'}</Box>
