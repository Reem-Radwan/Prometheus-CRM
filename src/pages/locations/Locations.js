...
//     }
//     Object.keys(filters).forEach((col) => { if (filters[col].length > 0) f = f.filter((row) => filters[col].includes(getVal(row, col))); });
//     if (sortColumn) {
//       f.sort((a, b) => {
...
//   const filteredPopoverValues = currentFilterColumn ? getUniqueVals(currentFilterColumn).filter((v) => String(v).toLowerCase().includes(filterSearchText.toLowerCase())) : [];

//   return { filtered, paginated, page, setPage, totalPages, sortColumn, sortDirection, handleSortClick, filterAnchorEl, filterOpen, currentFilterColumn, filters, filterSearchText, setFilterSearchText, globalSearch, setGlobalSearch, handleFilterClick, handleFilterClose, handleFilterToggle, handleSelectAll, handleClearAll, filteredPopoverValues };
// }
...
//             control={<Checkbox checked={currentCol ? filters[currentCol]?.includes(val) : false} onChange={() => onToggle(val)} size="small" sx={{ py: 0.3, '& .MuiSvgIcon-root': { fontSize: 17 } }} />}
//             label={val}
//             sx={{ display: 'block', mb: 0, ml: 0, mr: 0, '& .MuiFormControlLabel-label': { fontSize: '0.82rem', lineHeight: 1.5 }, '& .MuiCheckbox-root': { py: 0.3 } }} />
//         ))}
//       </Box>
//     </Popover>
//   );
...
//           sx={{ minWidth: '36px', height: '36px', borderRadius: '8px', fontWeight: 600, fontSize: '14px', backgroundColor: page === n ? 'primary.main' : 'transparent', color: page === n ? '#FFFFFF' : 'text.primary', border: '1px solid', borderColor: page === n ? 'primary.main' : '#E5E7EB', transition: 'all 0.3s ease', '&:hover': { backgroundColor: page === n ? 'primary.dark' : 'rgba(59,130,246,0.1)', transform: 'scale(1.1)' } }}>
//           {n + 1}
//         </IconButton>
//       ))}
...
// // ─── Sort + Filter header ─────────────────────────────────────────────────────
// const headerCellSx = { py: 1.5, px: 2, whiteSpace: 'nowrap', fontSize: '0.82rem', fontWeight: 700, color: '#374151', backgroundColor: '#F9FAFB', borderBottom: '2px solid #E5E7EB' };
// const stickyHeaderCellSx = { ...headerCellSx, position: { xs: 'sticky', md: 'static' }, left: { xs: 0, md: 'auto' }, zIndex: { xs: 3, md: 'auto' }, boxShadow: { xs: '2px 0 4px rgba(0,0,0,0.1)', md: 'none' } };
...
//           <IconButton size="small" onClick={() => onSort(column)} sx={{ p: 0.35, color: sortColumn === column ? 'primary.main' : '#9CA3AF', '&:hover': { backgroundColor: 'rgba(59,130,246,0.1)' } }}>
//             {sortColumn === column && sortDirection === 'asc' ? <ArrowUpwardIcon sx={{ fontSize: 16 }} /> : <ArrowDownwardIcon sx={{ fontSize: 16 }} />}
//           </IconButton>
//         </Tooltip>
//         <Tooltip title={`Filter ${label}`} arrow>
//           <IconButton size="small" onClick={(e) => onFilter(e, column)} sx={{ p: 0.35, color: filterCount > 0 ? 'primary.main' : '#9CA3AF', '&:hover': { backgroundColor: 'rgba(59,130,246,0.1)' } }}>
...
//   const branchCols = { id: 'ID', name: 'Branch Name', address: 'Address', status: 'Status' };
...
//   // ── Hide columns entirely empty on the current page ───────────────────────
//   const visibleColumns = useMemo(() => {
//     if (controls.paginated.length === 0) return Object.keys(branchCols);
//     return Object.keys(branchCols).filter(col =>
//       controls.paginated.some(row => {
//         const val = getBranchVal(row, col);
//         return val && val !== '-';
//       })
//     );
//   }, [controls.paginated, getBranchVal]);
...
//             <TextField fullWidth size="small" placeholder="Search branches by name or address..." value={controls.globalSearch} onChange={(e) => controls.setGlobalSearch(e.target.value)}
...
//                     <SortFilterHeader key={col} label={branchCols[col]} column={col} sticky={index === 0}
...
//                         {Object.values(controls.filters).flat().length > 0 || controls.globalSearch ? 'Try adjusting your search or filters' : 'Add your first branch'}
//                       </Typography>
//                       {Object.values(controls.filters).flat().length === 0 && !controls.globalSearch && (
//                         <Button variant="contained" size="small" startIcon={<AddIcon />} onClick={() => navigate('/locations/branches/create')}>New Branch</Button>
//                       )}
//                     </TableCell>
//                   </TableRow>
//                 ) : (
//                   controls.paginated.map((branch) => (
//                     <TableRow key={branch.id} hover sx={{ '&:last-child td': { borderBottom: 0 }, '&:hover': { backgroundColor: 'rgba(59,130,246,0.04)' } }}>
//                       {visibleColumns.map((col, idx) => (
//                         <TableCell key={col} sx={idx === 0 ? stickyBodyCellSx : { py: 1.5, px: 2 }}>
//                           {col === 'id' && (
//                             <Chip label={`#${branch.id}`} size="small" color="primary" variant="outlined" sx={{ fontWeight: 700, fontSize: '0.8rem', height: '28px', px: 0.5 }} />
//                           )}
//                           {col === 'name' && (
//                             <Typography sx={{ fontSize: '0.93rem', fontWeight: 600, color: '#111827' }}>{branch.name}</Typography>
//                           )}
//                           {col === 'address' && (
//                             <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
//                               <LocationOnIcon sx={{ fontSize: 16, color: '#9CA3AF', flexShrink: 0 }} />
//                               <Typography sx={{ fontSize: '0.9rem', color: '#4B5563' }}>{branch.address || '-'}</Typography>
//                             </Box>
//                           )}
//                           {col === 'status' && (
//                             <Chip label={branch.is_active ? 'Active' : 'Inactive'} size="small"
//                               sx={{ backgroundColor: branch.is_active ? '#DCFCE7' : '#FEE2E2', color: branch.is_active ? '#166534' : '#991B1B', fontWeight: 700, fontSize: '0.8rem', height: '28px', px: 0.5 }} />
//                           )}
//                         </TableCell>
//                       ))}
//                       <TableCell align="center" sx={{ py: 1.5, px: 2, whiteSpace: 'nowrap' }}>
//                         <Switch checked={branch.is_active} onChange={() => handleToggleActive(branch)} color="success" />
//                       </TableCell>
//                       <TableCell align="center" sx={{ py: 1.5, px: 2, whiteSpace: 'nowrap' }}>
//                         <Box sx={{ display: 'flex', gap: 0.75, justifyContent: 'center' }}>
//                           <Tooltip title="Edit" arrow>
//                             <IconButton size="small" onClick={() => navigate(`/locations/branches/edit/${branch.id}`)}
...
//   const siteCols = { id: 'ID', name: 'Project Name', code: 'Code', status: 'Status' };
...
//   // ── Hide columns entirely empty on the current page ───────────────────────
//   const visibleColumns = useMemo(() => {
//     if (controls.paginated.length === 0) return Object.keys(siteCols);
//     return Object.keys(siteCols).filter(col =>
//       controls.paginated.some(row => {
//         const val = getSiteVal(row, col);
//         return val && val !== '-';
//       })
//     );
//   }, [controls.paginated, getSiteVal]);
...
//         <Card>
//           <Box sx={{ p: 2, borderBottom: '1px solid #E5E7EB' }}>
//             <TextField fullWidth size="small" placeholder="Search project sites by name or code..." value={controls.globalSearch} onChange={(e) => controls.setGlobalSearch(e.target.value)}
//               InputProps={{ startAdornment: <InputAdornment position="start"><SearchIcon sx={{ fontSize: 18, color: 'text.disabled' }} /></InputAdornment> }}
//               sx={{ '& .MuiInputBase-root': { height: '38px', fontSize: '0.85rem' } }} />
//           </Box>
...
//                 <TableRow>
//                   {visibleColumns.map((col, index) => (
//                     <SortFilterHeader key={col} label={siteCols[col]} column={col} sticky={index === 0}
//                       sortColumn={controls.sortColumn} sortDirection={controls.sortDirection}
//                       filterCount={controls.filters[col]?.length || 0}
//                       onSort={controls.handleSortClick} onFilter={controls.handleFilterClick} />
//                   ))}
...
//                       <Typography color="text.secondary" sx={{ mb: 2, fontSize: '0.82rem' }}>
//                         {Object.values(controls.filters).flat().length > 0 || controls.globalSearch ? 'Try adjusting your search or filters' : 'Add your first project site'}
//                       </Typography>
//                       {Object.values(controls.filters).flat().length === 0 && !controls.globalSearch && (
//                         <Button variant="contained" size="small" startIcon={<AddIcon />} onClick={() => navigate('/locations/sites/create')}>New Project Site</Button>
//                       )}
//                     </TableCell>
//                   </TableRow>
//                 ) : (
//                   controls.paginated.map((site) => (
//                     <TableRow key={site.id} hover sx={{ '&:last-child td': { borderBottom: 0 }, '&:hover': { backgroundColor: 'rgba(59,130,246,0.04)' } }}>
//                       {visibleColumns.map((col, idx) => (
//                         <TableCell key={col} sx={idx === 0 ? stickyBodyCellSx : { py: 1.5, px: 2 }}>
//                           {col === 'id' && (
//                             <Chip label={`#${site.id}`} size="small" color="primary" variant="outlined" sx={{ fontWeight: 700, fontSize: '0.8rem', height: '28px', px: 0.5 }} />
//                           )}
//                           {col === 'name' && (
//                             <Typography sx={{ fontSize: '0.93rem', fontWeight: 600, color: '#111827' }}>{site.name}</Typography>
//                           )}
//                           {col === 'code' && (
//                             <Chip label={site.code || '-'} size="small"
//                               sx={{ backgroundColor: '#EFF6FF', color: '#1D4ED8', fontWeight: 700, fontSize: '0.8rem', height: '28px', px: 0.5, fontFamily: 'monospace' }} />
//                           )}
//                           {col === 'status' && (
//                             <Chip label={site.is_active ? 'Active' : 'Inactive'} size="small"
//                               sx={{ backgroundColor: site.is_active ? '#DCFCE7' : '#FEE2E2', color: site.is_active ? '#166534' : '#991B1B', fontWeight: 700, fontSize: '0.8rem', height: '28px', px: 0.5 }} />
//                           )}
//                         </TableCell>
//                       ))}
//                       <TableCell align="center" sx={{ py: 1.5, px: 2, whiteSpace: 'nowrap' }}>
//                         <Switch checked={site.is_active} onChange={() => handleToggleActive(site)} color="success" />
//                       </TableCell>
//                       <TableCell align="center" sx={{ py: 1.5, px: 2, whiteSpace: 'nowrap' }}>
//                         <Box sx={{ display: 'flex', gap: 0.75, justifyContent: 'center' }}>
//                           <Tooltip title="Edit" arrow>
//                             <IconButton size="small" onClick={() => navigate(`/locations/sites/edit/${site.id}`)}
//                               sx={{ p: 0.5, color: 'warning.main', transition: 'all 0.2s ease', '&:hover': { backgroundColor: 'rgba(245,158,11,0.1)', transform: 'scale(1.2)' } }}>
//                               <EditIcon sx={{ fontSize: 20 }} />
//                             </IconButton>
//                           </Tooltip>
...
//   const handleTabChange = (_, newVal) => { setSearchParams({ tab: newVal === 1 ? 'sites' : 'branches' }); };
...
//           breadcrumbs={[{ label: 'Home', href: '/' }, { label: 'Locations', active: true }]}
//           actions={
//             <Button variant="contained" startIcon={<AddIcon sx={{ display: { xs: 'none', sm: 'inline-flex' } }} />} size="large"
//               onClick={() => navigate(isSitesTab ? '/locations/sites/create' : '/locations/branches/create')}
...
//               <Box component="span" sx={{ display: { xs: 'none', sm: 'inline' } }}>{isSitesTab ? 'New Project Site' : 'New Branch'}</Box>
...
// ── Moved outside component to be a stable reference (fixes react-hooks/exhaustive-deps) ──
const branchCols = { id: 'ID', name: 'Branch Name', address: 'Address', status: 'Status' };
