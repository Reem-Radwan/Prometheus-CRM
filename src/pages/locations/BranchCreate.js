...
export default function BranchCreate() {
  ...
  const onSubmit = async (data) => {
    ...
    clearErrors();
    let hasErrors = false;
    if (!data.name?.trim()) { setError('name', { type: 'manual', message: 'Branch name is required' }); hasErrors = true; }
    if (!data.address?.trim()) { setError('address', { type: 'manual', message: 'Address is required' }); hasErrors = true; }
    if (hasErrors) return;
    try {
      const payload = { name: data.name.trim(), address: data.address.trim(), google_maps_url: data.google_maps_url?.trim() || null, is_active: data.is_active };
    ...
    } catch (err) { setSubmitError('Failed to create branch. Please try again.'); }
  };
              ...
              <Grid container spacing={3}>
                <Grid item xs={12}>
                  <Controller name="name" control={control} render={({ field }) => (
                    <TextField {...field} label="Branch Name" fullWidth required placeholder="e.g., New Cairo Sales Center" error={!!errors.name} helperText={errors.name?.message} />
                  )} />
                </Grid>
                <Grid item xs={12}>
                  <Controller name="address" control={control} render={({ field }) => (
                    <TextField {...field} label="Address" fullWidth required placeholder="e.g., 5th Settlement, New Cairo, Cairo" error={!!errors.address} helperText={errors.address?.message} />
                  )} />
                </Grid>
                <Grid item xs={12}>
                  <Controller name="google_maps_url" control={control} render={({ field }) => (
                    <TextField {...field} label="Google Maps URL" fullWidth placeholder="e.g., https://maps.google.com/?q=... (optional)" error={!!errors.google_maps_url} helperText={errors.google_maps_url?.message} />
                  )} />
                </Grid>
