...
export default function BranchEdit() {
  ...
  useEffect(() => {
    ...
    if (!branch) { showErrorToast('Branch not found'); setTimeout(() => navigate('/locations?tab=branches'), 2000); return; }
    setBranchData(branch);
    reset({ name: branch.name || '', address: branch.address || '', google_maps_url: branch.google_maps_url || '', is_active: branch.is_active ?? true });
  }, [id, navigate, reset]);
  ...
  const onSubmit = async (data) => {
    ...
    } catch (err) { setSubmitError('Failed to update branch. Please try again.'); }
  };
              ...
              <Grid container spacing={3}>
                <Grid item xs={12}>
                  <Controller name="name" control={control} render={({ field }) => (
                    <TextField {...field} label="Branch Name" fullWidth required error={!!errors.name} helperText={errors.name?.message} />
                  )} />
                </Grid>
                <Grid item xs={12}>
                  <Controller name="address" control={control} render={({ field }) => (
                    <TextField {...field} label="Address" fullWidth required error={!!errors.address} helperText={errors.address?.message} />
                  )} />
                </Grid>
                <Grid item xs={12}>
                  <Controller name="google_maps_url" control={control} render={({ field }) => (
                    <TextField {...field} label="Google Maps URL" fullWidth placeholder="https://maps.google.com/?q=... (optional)" error={!!errors.google_maps_url} helperText={errors.google_maps_url?.message} />
                  )} />
                </Grid>
