...
export default function ProjectSiteEdit() {
  ...
  useEffect(() => {
    ...
    if (!site) { showErrorToast('Project site not found'); setTimeout(() => navigate('/locations?tab=sites'), 2000); return; }
    setSiteData(site);
    reset({ name: site.name || '', code: site.code || '', is_active: site.is_active ?? true });
  }, [id, navigate, reset]);
  ...
  const onSubmit = async (data) => {
    ...
    } catch (err) { setSubmitError('Failed to update project site. Please try again.'); }
  };
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} md={4}>
                  <Controller name="code" control={control} render={({ field }) => (
                    <TextField {...field} label="Location Code" fullWidth required inputProps={{ style: { textTransform: 'uppercase' } }} error={!!errors.code} helperText={errors.code?.message || 'Unique identifier'} />
                  )} />
                </Grid>
