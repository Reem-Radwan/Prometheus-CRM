...
// No ID — name is first (sticky on mobile)
const columnLabels = {
  name:        'Name',
  category:    'Category',
...
export default function LeadSources() {
  ...
  const [filters, setFilters]                         = useState({ name: [], category: [], system_code: [] });
  ...
  const filteredSources = useMemo(() => {
    ...
    Object.keys(filters).forEach(col => {
      if (filters[col].length > 0) result = result.filter(row => filters[col].includes(getColumnValue(row, col)));
    });
    if (globalSearch.trim()) {
      const q = globalSearch.trim().toLowerCase();
      result = result.filter(row => Object.keys(columnLabels).some(col => String(getColumnValue(row, col)).toLowerCase().includes(q)));
    }
    if (sortColumn) {
      result.sort((a, b) => {
  ...
  const getUniqueColumnValues = useCallback(col => {
    ...
    Object.keys(filters).forEach(c => {
      if (c !== col && filters[c].length > 0) data = data.filter(row => filters[c].includes(getColumnValue(row, c)));
    });
    return [...new Set(data.map(row => getColumnValue(row, col)))].sort();
  }, [sources, filters, getColumnValue]);

  const paginatedSources = filteredSources.slice(page * pageSize, (page + 1) * pageSize);
  ...
  const handleDelete = async source => {
    const result = await showDeleteConfirmation('Delete Lead Source?', `Delete "${source.name}"? This may affect existing leads using this source.`);
    ...
    <Fade in timeout={500}>
      <Box>
        <PageHeader
          ...
          breadcrumbs={[{ label: 'Home', href: '/' }, { label: 'Leads', href: '/leads' }, { label: 'Lead Sources', active: true }]}
          actions={
        ...
        <Grow in timeout={850}>
          <Card sx={{ mb: 3, p: 2.5 }}>
            <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
              <TextField fullWidth size="small" placeholder="Search by name, category, or system code…" value={globalSearch}
              ...
              {hasAnyFilter && (
                <Button size="small" variant="outlined" color="error" startIcon={<CloseIcon fontSize="small" />}
                  onClick={() => { setGlobalSearch(''); setFilters({ name: [], category: [], system_code: [] }); }}
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                          ...
                          <Tooltip title={`Sort by ${columnLabels[col]}`} arrow TransitionComponent={Fade} TransitionProps={{ timeout: 300 }}>
                            <IconButton size="small" onClick={() => handleSortClick(col)}
                              ...
                              {sortColumn === col && sortDirection === 'asc' ? <ArrowUpwardIcon sx={{ fontSize: 18 }} /> : <ArrowDownwardIcon sx={{ fontSize: 18 }} />}
                            </IconButton>
                          </Tooltip>
                  ...
                  {paginatedSources.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={4} align="center" sx={{ py: 8 }}>
                        ...
                        {!hasAnyFilter && (
                          <Button variant="contained" startIcon={<AddIcon />} onClick={() => navigate('/lead-sources/create')}>Create Source</Button>
                        )}
                      </TableCell>
                    </TableRow>
                  ) : (
        ...
        <Popover open={filterOpen} anchorEl={filterAnchorEl} onClose={handleFilterClose}
          ...
          <TextField fullWidth size="small" placeholder="Search..." value={filterSearchText} onChange={e => setFilterSearchText(e.target.value)}
