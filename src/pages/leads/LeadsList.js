...
// ✅ Sticky cell styles for name column on mobile
        ...
        return (
          <Box key={year} sx={{ mb: 0.5 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', borderRadius: '8px', '&:hover': { backgroundColor: 'rgba(59,130,246,0.06)' }, cursor: 'pointer', pr: 0.5 }}>
              <Checkbox size="small" checked={isYearChecked(year)} indeterminate={isYearIndeterminate(year)}
              ...
              <IconButton size="small" onClick={() => toggleYear(year)} sx={{ p: 0.25 }}>
                {isExpanded
                ...
                {months.map(monthIdx => {
                  ...
                  return (
                    <FormControlLabel key={key}
                      control={<Checkbox size="small" checked={selectedDates.includes(key)}
...
export default function LeadsList() {
  ...
  const [filters, setFilters] = useState({
    name: [], phone: [], email: [], national_id: [], source: [], created_at: [],
  });
  ...
  const allSources = useMemo(() => DataService.getLeadSources().map(s => ({ id: s.id, name: s.name })), []);
  ...
  const activeMatchType = useMemo(() => {
    ...
    if (leads.some(l => (l.phone || '').toLowerCase().includes(q) || normalizePhone(l.phone || '').includes(normalizedQ))) return 'phone';
  ...
  const filteredLeads = useMemo(() => {
    ...
    if (sourceFilter) result = result.filter(l => l.source_id === sourceFilter);
    ...
    if (globalSearch.trim()) {
      result = result.filter((lead) => {
        ...
        switch (activeMatchType) {
          case 'name':        return `${lead.first_name} ${lead.last_name}`.toLowerCase().includes(q);
          case 'phone':       return (lead.phone || '').toLowerCase().includes(q) || normalizePhone(lead.phone || '').includes(normalizedQ);
          case 'email':       return (lead.email || '').toLowerCase().includes(q);
          case 'national_id': return String(lead.national_id || '').includes(q);
  ...
  const handleFilterClick  = (event, column) => { setFilterAnchorEl(event.currentTarget); setCurrentFilterColumn(column); setSearchText(''); };
  const handleFilterClose  = () => { setFilterAnchorEl(null); setCurrentFilterColumn(null); setSearchText(''); };
  const handleFilterToggle = (value) => {
    const col = currentFilterColumn; const cur = filters[col];
  ...
  const visibleColumns = useMemo(() => {
    ...
    return Object.keys(columnLabels).filter(col =>
      paginatedLeads.some(lead => { const val = getColumnValue(lead, col); return val && val !== '-'; })
    );
  }, [paginatedLeads, getColumnValue]);

  useEffect(() => { setPage(0); }, [filters, sortColumn, sortDirection, globalSearch, sourceFilter, pageSize]);
  ...
  const activeFilterCount = Object.entries(filters).reduce((acc, [, val]) => acc + (val.length > 0 ? 1 : 0), 0) + (sourceFilter ? 1 : 0);
        ...
        <PageHeader
          ...
          actions={
            <Button variant="contained"
              ...
              size="large" onClick={() => navigate('/leads/create')}
          ...
          <Card sx={{ mb: 3, p: 2.5 }}>
            <Box sx={{ display: 'flex', gap: 2, alignItems: 'flex-start', flexDirection: { xs: 'column', sm: 'row' } }}>
                ...
                {globalSearch.trim() && activeMatchType !== 'name' && (
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 1, flexWrap: 'wrap' }}>
                    {['phone', 'email', 'national_id'].map((type, idx) => {
                      ...
                      const isPassed = (type === 'email' && activeMatchType === 'national_id') || (type === 'phone' && ['email', 'national_id'].includes(activeMatchType));
                      return (
                        <React.Fragment key={type}>
                          ...
                          {idx < 2 && <Typography variant="caption" sx={{ color: '#9CA3AF', fontSize: '11px' }}>→</Typography>}
                        </React.Fragment>
                      );
                    })}
                    ...
                    {!activeMatchType && <Typography variant="caption" sx={{ color: '#EF4444', fontSize: '11px' }}>No results found</Typography>}
                  </Box>
                )}
              </Box>
              ...
              <FormControl size="small" sx={{ minWidth: 200, width: { xs: '100%', sm: 'auto' } }}>
                ...
                <Select value={sourceFilter} label="Filter by Source" onChange={e => setSourceFilter(e.target.value)} sx={{ borderRadius: '10px' }}>
                  <MenuItem value=""><em>All Sources</em></MenuItem>
                  {allSources.map(source => <MenuItem key={source.id} value={source.id}>{source.name}</MenuItem>)}
                </Select>
              </FormControl>
              ...
              {(globalSearch || sourceFilter || Object.values(filters).flat().length > 0) && (
                <Button size="small" variant="outlined" color="error" startIcon={<CloseIcon fontSize="small" />}
                  onClick={() => { setGlobalSearch(''); setSourceFilter(''); setFilters({ name: [], phone: [], email: [], national_id: [], source: [], created_at: [] }); }}
        ...
        <Grow in timeout={900}>
          <Card>
                ...
                <TableHead>
                  <TableRow>
                    {visibleColumns.map((column) => (
                      // ✅ Name column gets sticky header on mobile
                      <TableCell key={column} sx={column === 'name' ? stickyNameHeaderSx : { py: 1.5, px: 2, whiteSpace: 'nowrap' }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                          ...
                          <Tooltip title={`Filter ${columnLabels[column]}`} arrow TransitionComponent={Fade} TransitionProps={{ timeout: 300 }}>
                            <IconButton size="small" onClick={(e) => handleFilterClick(e, column)}
                              sx={{ p: 0.5, color: (column === 'created_at' ? filters.created_at.length > 0 : filters[column]?.length > 0) ? 'primary.main' : 'inherit', transition: 'all 0.2s ease', '&:hover': { transform: 'scale(1.15)', backgroundColor: 'rgba(59, 130, 246, 0.1)' } }}>
                    ...
                    <TableRow>
                      <TableCell colSpan={visibleColumns.length + 1} align="center" sx={{ py: 8 }}>
                        <Box>
                          ...
                          {!globalSearch && !sourceFilter && Object.values(filters).flat().length === 0 && (
                            <Button variant="contained" startIcon={<AddIcon />} onClick={() => navigate('/leads/create')}>Create Lead</Button>
                          )}
                        </Box>
                      </TableCell>
                    </TableRow>
                    ...
                    paginatedLeads.map((lead) => (
                      <TableRow key={lead.id} hover
                        ...
                        {visibleColumns.map(col => (
                          // ✅ Name column gets sticky body style on mobile
                          <TableCell key={col} sx={col === 'name' ? stickyNameBodySx : { py: 1.5, px: 2 }}>
                            {col === 'name' && <Typography variant="body2" fontWeight={600} noWrap>{lead.first_name} {lead.last_name}</Typography>}
                            {col === 'phone' && <Typography variant="body2" sx={{ fontFamily: 'monospace' }} noWrap>{lead.phone || '-'}</Typography>}
                            {col === 'email' && (lead.email ? <Typography variant="body2" noWrap>{lead.email}</Typography> : <Typography variant="body2" color="text.disabled">-</Typography>)}
                            {col === 'national_id' && (lead.national_id ? <Typography variant="body2" sx={{ fontFamily: 'monospace' }} noWrap>{lead.national_id}</Typography> : <Typography variant="body2" color="text.disabled">-</Typography>)}
                            {col === 'source' && <Chip label={getLeadSource(lead.source_id)} size="small" sx={{ backgroundColor: '#E0E7FF', color: '#3730A3', fontWeight: 600, fontSize: '12px' }} />}
                            {col === 'created_at' && <Typography variant="body2" color="text.secondary" noWrap>{lead.created_at || '-'}</Typography>}
                          </TableCell>
                        ))}
            ...
            <PaginationWithSize
              ...
              totalCount={filteredLeads.length} pageSize={pageSize}
          ...
          {currentFilterColumn === 'created_at' ? (
            <>
              ...
              <Box sx={{ maxHeight: 300, overflowY: 'auto' }}>
                <DateFilterContent leads={leads} filters={filters} setFilters={setFilters} />
              </Box>
            </>
          ) : (
            <>
              ...
              <Box sx={{ maxHeight: 220, overflowY: 'auto' }}>
                {filteredColumnValues.map((value) => (
                  <FormControlLabel key={value}
                    control={<Checkbox checked={currentFilterColumn ? filters[currentFilterColumn]?.includes(value) : false}
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                {[
                  ...
                  <Box key={field.label}>
                    ...
                    <Typography variant="body1" fontWeight={field.label === 'Name' ? 600 : 400}
