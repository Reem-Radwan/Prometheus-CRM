  ...
  return (
    <Box sx={{ pt: 0.5 }}>
      <Box sx={{ display: 'flex', gap: 1, flexDirection: 'column' }}>
        <Box>
          <TextField fullWidth size="small" label="Min Budget" type="number"
        ...
        <Box>
          <TextField fullWidth size="small" label="Max Budget" type="number"
...
function useTableControls(data, columnDefs, getVal) {
  ...
  const [dateFilters, setDateFilters] = useState(
    Object.fromEntries(Object.keys(columnDefs).filter(k => DATE_COLUMNS.includes(k)).map(k => [k, []]))
  );
  ...
  const getUniqueVals = useCallback((col) => {
    ...
    Object.keys(filters).forEach((c) => {
      ...
      if (c !== col && filters[c].length > 0) d = d.filter((row) => filters[c].includes(getVal(row, c)));
    });
  ...
  useEffect(() => { setPage(0); }, [filters, dateFilters, budgetFilter, sortColumn, sortDirection, globalSearch, pageSize]);
  ...
  const filteredPopoverValues = currentFilterColumn && !DATE_COLUMNS.includes(currentFilterColumn) && currentFilterColumn !== 'budget'
    ? getUniqueVals(currentFilterColumn).filter((v) => String(v).toLowerCase().includes(filterSearchText.toLowerCase()))
    : [];
...
function FilterPopover({
  ...
  return (
    <Popover open={open} anchorEl={anchorEl} onClose={onClose}
      ...
      {isBudgetCol && (
        <BudgetRangeFilter rows={allRows} budgetFilter={budgetFilter}
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 0 }}>
        ...
        <Tooltip title={`Sort by ${label}`} arrow>
          <IconButton size="small" onClick={() => onSort(column)}
            sx={{ p: 0.3, color: sortColumn === column ? 'primary.main' : 'text.secondary', '&:hover': { backgroundColor: 'rgba(59,130,246,0.1)' } }}>
            {sortColumn === column && sortDirection === 'asc' ? <ArrowUpwardIcon sx={{ fontSize: 15 }} /> : <ArrowDownwardIcon sx={{ fontSize: 15 }} />}
          </IconButton>
        </Tooltip>
        <Tooltip title={`Filter ${label}`} arrow>
          <IconButton size="small" onClick={(e) => onFilter(e, column)}
            sx={{ p: 0.3, color: filterActive ? 'primary.main' : 'text.secondary', '&:hover': { backgroundColor: 'rgba(59,130,246,0.1)' } }}>
...
function CampaignsTab() {
  ...
  const visibleColumns = useMemo(() => {
    ...
    return Object.keys(campaignCols).filter(col =>
      controls.paginated.some(row => { const val = getCampaignVal(row, col); return val && val !== '-'; })
    );
  ...
  return (
    <Box>
      ...
      <Grow in timeout={900}>
        <Card>
          <Box sx={{ p: 1.5, borderBottom: '1px solid #E5E7EB' }}>
            <TextField fullWidth size="small" placeholder="Search campaigns by name, platform, status..."
              value={controls.globalSearch} onChange={(e) => controls.setGlobalSearch(e.target.value)}
              ...
              <TableHead>
                <TableRow>
                  {visibleColumns.map((col, index) => (
                    <SortFilterHeader key={col} label={campaignCols[col]} column={col}
                      sticky={index === 0}
                  ...
                  <TableRow>
                    <TableCell colSpan={visibleColumns.length + 2} align="center" sx={{ py: 6 }}>
                      ...
                      {Object.values(controls.filters).flat().length === 0 && !controls.globalSearch && (
                        <Button variant="contained" size="small" startIcon={<AddIcon />} onClick={() => navigate('/marketing/campaigns/create')}>Create Campaign</Button>
                      )}
                    </TableCell>
                  </TableRow>
                ) : (
      ...
      <FilterPopover
        ...
        dateFilters={controls.dateFilters} setDateFilters={controls.setDateFilters}
        isBudgetCol={currentColIsBudget}
        budgetFilter={controls.budgetFilter} setBudgetFilter={controls.setBudgetFilter}
...
function EventsTab() {
  ...
  const visibleColumns = useMemo(() => {
    ...
    return Object.keys(eventCols).filter(col =>
      controls.paginated.some(row => { const val = getEventVal(row, col); return val && val !== '-'; })
    );
  ...
  return (
    <Box>
      ...
      <Grow in timeout={900}>
        <Card>
          <Box sx={{ p: 1.5, borderBottom: '1px solid #E5E7EB' }}>
            <TextField fullWidth size="small" placeholder="Search events by name, status..."
              value={controls.globalSearch} onChange={(e) => controls.setGlobalSearch(e.target.value)}
              InputProps={{ startAdornment: <Box component="span" sx={{ display: 'flex', mr: 1 }}><SearchIcon sx={{ fontSize: 18, color: 'text.disabled' }} /></Box> }}
              sx={{ '& .MuiInputBase-root': { height: '36px', fontSize: '0.85rem' } }} />
          </Box>
          ...
          <TableContainer sx={{ overflowX: 'auto' }}>
            <Table size="small" sx={{ minWidth: 480 }}>
              <TableHead>
                <TableRow>
                  {visibleColumns.map((col, index) => (
                    <SortFilterHeader key={col} label={eventCols[col]} column={col}
                      sticky={index === 0}
                      sortColumn={controls.sortColumn} sortDirection={controls.sortDirection}
                      filterActive={controls.isFilterActive(col)}
                      onSort={controls.handleSortClick} onFilter={controls.handleFilterClick} />
                ...
                {controls.paginated.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={visibleColumns.length + 2} align="center" sx={{ py: 6 }}>
                      ...
                      {Object.values(controls.filters).flat().length === 0 && !controls.globalSearch && (
                        <Button variant="contained" size="small" startIcon={<AddIcon />} onClick={() => navigate('/marketing/events/create')}>Create Event</Button>
                      )}
                    </TableCell>
                  </TableRow>
                ) : (
...
export default function Marketing() {
  ...
  const handleTabChange = (_, newVal) => {
    setSearchParams({ tab: newVal === 1 ? 'events' : 'campaigns' }, { replace: true });
  };
          ...
          actions={
            <Button variant="contained"
              ...
              <Box component="span" sx={{ display: { xs: 'none', sm: 'inline' } }}>
                {isEventsTab ? 'Create Event' : 'Create Campaign'}
              </Box>
