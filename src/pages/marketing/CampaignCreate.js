...
export default function CampaignCreate() {
  ...
  const onSubmit = async (data) => {
    ...
    if (!data.name?.trim()) {
      setError('name', { type: 'manual', message: 'Campaign name is required' });
      hasErrors = true;
    }
    if (!data.platform_id) {
      setError('platform_id', { type: 'manual', message: 'Platform is required' });
      hasErrors = true;
    }
    ...
    try {
      ...
      MarketingDataService.createCampaign(payload);
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <Grid container spacing={3}>
                <Grid item xs={12} md={8}>
                  <Controller name="name" control={control} render={({ field }) => (
                    <TextField {...field} label="Campaign Name" fullWidth required
                ...
                <Grid item xs={12} md={4}>
                  <Controller name="platform_id" control={control} render={({ field }) => (
                    <TextField {...field} select label="Platform" fullWidth required
                      error={!!errors.platform_id} helperText={errors.platform_id?.message}
                        ...
                        <MenuItem key={p.id} value={p.id}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            <Box sx={{ width: 10, height: 10, borderRadius: '50%',
                              backgroundColor: getPlatformStyle(p.id).color }} />
                            {p.name}
                          </Box>
                        </MenuItem>
                      ))}
                ...
                <Grid item xs={12} md={4}>
                  <Controller name="budget" control={control} render={({ field }) => (
                    <TextField {...field} label="Budget (EGP)" type="number" fullWidth
            ...
            {platformStyle && (
              <Paper elevation={0} sx={{ p: 3, mb: 4,
                ...
                border: `1px solid ${platformStyle.color}30`,
                borderRadius: '12px' }}>
            ...
            <Box sx={{ display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
              <Button variant="outlined" size="large" onClick={() => navigate('/marketing?tab=campaigns')} disabled={isSubmitting} sx={{ minWidth: '120px' }}>
                Cancel
              </Button>
