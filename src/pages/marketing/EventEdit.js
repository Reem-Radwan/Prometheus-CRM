...
export default function EventEdit() {
  ...
  useEffect(() => {
    ...
    setEventData(event);
    reset({ name: event.name || '', start_date: event.start_date || '', end_date: event.end_date || '', is_active: event.is_active ?? true });
  }, [id, navigate, reset]);
  ...
  const onSubmit = async (data) => {
    ...
    clearErrors();
    let hasErrors = false;
    if (!data.name?.trim()) { setError('name', { type: 'manual', message: 'Event name is required' }); hasErrors = true; }
    if (!data.start_date) { setError('start_date', { type: 'manual', message: 'Start date is required' }); hasErrors = true; }
    if (data.start_date && data.end_date && data.end_date < data.start_date) { setError('end_date', { type: 'manual', message: 'End date must be on or after start date' }); hasErrors = true; }
    if (hasErrors) return;
    try {
      MarketingDataService.updateEvent(Number(id), { name: data.name.trim(), start_date: data.start_date, end_date: data.end_date || null, is_active: data.is_active });
              ...
              <Grid container spacing={3}>
                <Grid item xs={12}>
                  <Controller name="name" control={control} render={({ field }) => (
                    <TextField {...field} label="Event Name" fullWidth required error={!!errors.name} helperText={errors.name?.message} />
                  )} />
                </Grid>
                <Grid item xs={12} md={6}>
                  <Controller name="start_date" control={control} render={({ field }) => (
                    <TextField {...field} label="Start Date" type="date" fullWidth required InputLabelProps={{ shrink: true }} error={!!errors.start_date} helperText={errors.start_date?.message} />
                  )} />
                </Grid>
                <Grid item xs={12} md={6}>
                  <Controller name="end_date" control={control} render={({ field }) => (
                    <TextField {...field} label="End Date" type="date" fullWidth InputLabelProps={{ shrink: true }} error={!!errors.end_date} helperText={errors.end_date?.message || 'Optional – leave blank for single-day events'} />
                  )} />
                </Grid>
                <Grid item xs={12} md={6}>
