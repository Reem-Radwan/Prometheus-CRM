...
//     id: [], name: [], type: [], phone: [], company_name: [], commission: [], status: [],
//   });
...
//   // ── Hide columns that are entirely empty / '-' on the current page ─────────
//   const visibleColumns = useMemo(() => {
//     if (paginatedPartners.length === 0) return Object.keys(columnLabels);
//     return Object.keys(columnLabels).filter(col =>
//       paginatedPartners.some(partner => {
//         const val = getColumnValue(partner, col);
//         return val && val !== '-';
//       })
//     );
...
//                   onClick={() => { setGlobalSearch(''); setFilters({ id: [], name: [], type: [], phone: [], company_name: [], commission: [], status: [] }); }}
...
//                       <TableCell
//                         key={column}
//                         sx={{ py: 1.5, px: 2, whiteSpace: 'nowrap', ...(index === 0 && stickyHeaderSx) }}
//                       >
...
//                             )}
//                             {column === 'phone' && (
//                               <Typography variant="body2" sx={{ fontFamily: 'monospace' }} noWrap>{partner.phone}</Typography>
//                             )}
//                             {column === 'company_name' && (
//                               partner.company_name
//                                 ? <Typography variant="body2" noWrap>{partner.company_name}</Typography>
//                                 : <Typography variant="body2" color="text.disabled">-</Typography>
//                             )}
//                             {column === 'commission' && (
//                               partner.commission
//                                 ? <Chip label={`${partner.commission}%`} size="small" sx={{ backgroundColor: '#DCFCE7', color: '#166534', fontWeight: 600, fontSize: '12px' }} />
//                                 : <Typography variant="body2" color="text.disabled">-</Typography>
//                             )}
//                             {column === 'status' && (
...
//                   <IconButton key={pageNum} size="small" onClick={() => setPage(pageNum)}
//                     sx={{ minWidth: '36px', height: '36px', borderRadius: '8px', fontWeight: 600, fontSize: '14px', backgroundColor: page === pageNum ? 'primary.main' : 'transparent', color: page === pageNum ? '#FFFFFF' : 'text.primary', border: '1px solid', borderColor: page === pageNum ? 'primary.main' : '#E5E7EB', transition: 'all 0.3s ease', '&:hover': { backgroundColor: page === pageNum ? 'primary.dark' : 'rgba(59, 130, 246, 0.1)', transform: 'scale(1.1)' } }}>
//                     {pageNum + 1}
//                   </IconButton>
//                 ))}
